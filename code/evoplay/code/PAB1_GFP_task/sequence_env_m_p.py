...
class Seq_env(object):
    ...
    def __init__(
        ...
        self._state = string_to_one_hot(self.seq,
                                        self.alphabet).astype(np.float32)

        self.init_state = string_to_one_hot(self.seq,
                                            self.alphabet).astype(np.float32)
        self.previous_init_state = string_to_one_hot(self.seq,
                                                     self.alphabet).astype(
                                                         np.float32
                                                     )
    ...
    def init_seq_state(self):  #start_player=0
        ...
        # remove the mutation that do not change the sequence
        for i, a in enumerate(combo):
            self.availables.remove(self.vocab_size * i + AAS.index(a))

        # remove the mutation that has already been explored
...
class Mutate(object):
    ...
    def start_mutating(self, mutater, is_shown=0, temp=1e-3):  #mutater,
        ...
        print(
            "starting sequence: {}".format(self.Seq_env.init_combo)
        )  # seq_env.init_state
        print("starting fitness: %f" % (self.Seq_env._state_fitness))
        generated_seqs = []
        ...
        while True:
            ...
            if move:
                ...
                generated_seqs.append(
                    one_hot_to_string(self.Seq_env._state, AAS)
                )

                fit_result.append(self.Seq_env._state_fitness)
                print("move_fitness: %f" % (self.Seq_env._state_fitness))
                print("episode_seq len: %d" % (len(self.Seq_env.episode_seqs)))
                print(
                    "move & playout dict len: %d" %
